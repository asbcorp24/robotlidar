...
function renderMaps(maps, settings) {
  ...
  list.innerHTML = maps.map((map) => `
    <article class="map-item">
      ${map.image_exists
        ? `<img class="map-preview" src="/api/maps/${encodeURIComponent(map.name)}/preview.png?t=${map.modified_at}" alt="Карта ${map.name}">`
        : '<div class="map-preview empty">Нет изображения карты</div>'}
      <div class="map-body">
        <div class="map-title">
          <strong title="${map.name}">${map.name}</strong>
          ${map.default ? '<span class="default-tag">Основная</span>' : ''}
        </div>
        <div class="map-meta">
          Изменена: ${formatDate(map.modified_at)}<br>
          Разрешение: ${map.resolution ?? '—'} м/пиксель
        </div>
  ...
  $$('[data-select-map]').forEach((button) => {
  ...
  $$('[data-default-map]').forEach((button) => {
    button.addEventListener('click', async () => {
      try {
        const result = await request('/api/maps/default', {
          method: 'POST', body: JSON.stringify({ name: button.dataset.defaultMap }),
        });
...
async function sendDrive(action) {
  try {
    await request('/api/drive', { method: 'POST', body: JSON.stringify({ action }) });
  } catch (error) {
    stopDrive();
...
$$('.drive-button').forEach((button) => {
  ...
  if (action === 'stop') {
    button.addEventListener('click', () => stopDrive(true));
    return;
  }
  ...
  ['pointerup', 'pointercancel', 'lostpointercapture', 'pointerleave'].forEach((name) => {
    button.addEventListener(name, () => stopDrive(true));
  });
});
...
$('#startMapping').addEventListener('click', async () => {
  ...
  try {
    await request('/api/mode/mapping', { method: 'POST' });
    toast('Картографирование запускается', 'success');
  } catch (error) { toast(error.message, 'error'); }
});
...
$('#startNavigation').addEventListener('click', async () => {
  ...
  try {
    await request('/api/mode/navigation', {
      method: 'POST', body: JSON.stringify({ map_name: selectedMap }),
    });
    toast(`Запускается карта: ${selectedMap}`, 'success');
  } catch (error) { toast(error.message, 'error'); }
});
...
$('#stopMode').addEventListener('click', async () => {
  try {
    await request('/api/mode/stop', { method: 'POST' });
    toast('ROS-режим остановлен', 'success');
  } catch (error) { toast(error.message, 'error'); }
});

const routeAction = async (operation, successMessage) => {
  try {
    const data = await request(`/api/route/${operation}`, { method: 'POST' });
...
$('#saveMap').addEventListener('click', async () => {
  ...
  try {
    await request('/api/maps/save', {
      method: 'POST',
      body: JSON.stringify({ name, set_default: $('#saveAsDefault').checked }),
    });
    $('#newMapName').value = '';
    await refreshMaps();
  ...
  try {
    const data = await request('/api/settings', {
      ...
      body: JSON.stringify({
        ...
        startup_mode: $('#startupMode').value,
      }),
    });
    lastSettings = data.settings;
