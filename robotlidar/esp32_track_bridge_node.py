#!/usr/bin/env python3
"""Bridge ROS 2 drive, actuator, brush, auxiliary motor and safety telemetry to ESP32."""
...
class Esp32TrackBridgeNode(Node):
    """Send ROS commands and receive ESP32 track/Hall/RC/aux/safety telemetry."""
    ...
    def __init__(self) -> None:
        ...
        self.aux_command_timeout_sec = float(self.get_parameter('aux_command_timeout_sec').value)
        ...
        self.pulse_rate_publisher = self.create_publisher(Float32MultiArray, '/drive/hall_pps', 20)
        self.direction_publisher = self.create_publisher(Int8MultiArray, '/drive/direction', 20)
        self.actuator_state_publisher = self.create_publisher(Int8, '/actuator/state', 20)
        self.brush_state_publisher = self.create_publisher(Float32MultiArray, '/brush/state', 20)
        self.aux_motor_state_publisher = self.create_publisher(Float32MultiArray, '/aux_motor/state', 20)
        self.ultrasonic_distance_publisher = self.create_publisher(Float32, '/safety/ultrasonic_distance', 20)
        self.ultrasonic_stop_publisher = self.create_publisher(Bool, '/safety/obstacle_stop', 20)
        self.ultrasonic_status_publisher = self.create_publisher(String, '/safety/ultrasonic_status', 20)

        self.create_subscription(Twist, '/cmd_vel', self._cmd_vel_callback, 20)
        self.create_subscription(Int8, '/actuator/command', self._actuator_command_callback, 20)
        self.create_subscription(Float32, '/brush/command', self._brush_command_callback, 20)
        self.create_subscription(Float32, '/aux_motor/command', self._aux_motor_command_callback, 20)
        ...
        self._open_serial()
        if self.auto_arm:
            self.get_logger().warning('auto_arm is enabled; verify lifted tracks and emergency stop')
        ...
        self.get_logger().info(
            ...
            'safety=/safety/ultrasonic_distance,/safety/obstacle_stop'
            % ('DRY-RUN' if self.dry_run else 'HARDWARE', self.serial_port, self.baud_rate)
        )
    ...
    def _open_serial(self) -> None:
        ...
        self._reader_thread = threading.Thread(target=self._reader_loop, name='esp32-track-reader', daemon=True)
    ...
    def _reader_loop(self) -> None:
        while not self._reader_stop.is_set():
            ...
            if not raw:
                continue
            try:
                self._process_line(raw.decode('ascii', errors='strict').strip())
            except Exception as exc:
    ...
    def _process_line(self, line: str) -> None:
        ...
        elif fields[0] == 'USONIC':
            self._handle_ultrasonic_telemetry(fields)
        elif fields[0] == 'ACK':
            with self._lock:
    ...
    def _handle_ultrasonic_telemetry(self, fields: list[str]) -> None:
        if len(fields) < 7:
            raise ValueError(f'USONIC expected 7 fields, got {len(fields)}')
        distance_mm = max(0, int(fields[2]))
        telemetry = {
            'ultrasonic_millis': int(fields[1]),
            'ultrasonic_distance_mm': distance_mm,
            'ultrasonic_valid': bool(int(fields[3])),
            'ultrasonic_near': bool(int(fields[4])),
            'ultrasonic_stop': bool(int(fields[5])),
            'ultrasonic_emergency': bool(int(fields[6])),
            'ultrasonic_received_at': time.time(),
        }
        with self._lock:
        ...
        distance = Float32()
        distance.data = float(distance_mm) / 1000.0
        self.ultrasonic_distance_publisher.publish(distance)

        stop = Bool()
        stop.data = telemetry['ultrasonic_stop']
        self.ultrasonic_stop_publisher.publish(stop)

        status = String()
        status.data = json.dumps({
            'distance_mm': distance_mm,
            'distance_cm': round(distance_mm / 10.0, 1),
            'valid': telemetry['ultrasonic_valid'],
            'near': telemetry['ultrasonic_near'],
            'stop': telemetry['ultrasonic_stop'],
            'emergency': telemetry['ultrasonic_emergency'],
        }, ensure_ascii=False)
        self.ultrasonic_status_publisher.publish(status)
    ...
    def _handle_telemetry(self, fields: list[str]) -> None:
        ...
        telemetry = {
            'millis': int(fields[1]), 'armed': bool(int(fields[2])), 'estop_ok': bool(int(fields[3])),
            'target_left': int(fields[4]), 'target_right': int(fields[5]),
            'actual_left': int(fields[6]), 'actual_right': int(fields[7]),
            'ticks_left': int(fields[8]), 'ticks_right': int(fields[9]),
            'pps_left': float(fields[10]), 'pps_right': float(fields[11]),
            'watchdog': bool(int(fields[12])), 'received_at': time.time(),
        }
        if len(fields) >= 19:
            telemetry.update({
                'control_mode': fields[13], 'rc_ch1_us': int(fields[14]), 'rc_ch2_us': int(fields[15]),
                'rc_mode_us': int(fields[16]), 'rc_arm_us': int(fields[17]), 'rc_valid': bool(int(fields[18])),
            })
        if len(fields) >= 20:
            telemetry['rc_input_mode'] = fields[19]
        if len(fields) >= 24:
            telemetry.update({
                'actuator_rc_us': int(fields[20]), 'actuator_rc_valid': bool(int(fields[21])),
                'actuator_direction': int(fields[22]), 'actuator_timeout': bool(int(fields[23])),
            })
        if len(fields) >= 30:
            telemetry.update({
                'brush_rc_us': int(fields[24]), 'brush_rc_valid': bool(int(fields[25])),
                'brush_ros_command': int(fields[26]), 'brush_throttle_mv': int(fields[27]),
                'brush_brake': bool(int(fields[28])), 'brush_mcp_ready': bool(int(fields[29])),
            })
        with self._lock:
            preserved = {
        ...
        ticks = Int64MultiArray(); ticks.data = [telemetry['ticks_left'], telemetry['ticks_right']]; self.tick_publisher.publish(ticks)
        pps = Float32MultiArray(); pps.data = [telemetry['pps_left'], telemetry['pps_right']]; self.pulse_rate_publisher.publish(pps)
        direction = Int8MultiArray(); direction.data = [self._sign(telemetry['actual_left']), self._sign(telemetry['actual_right'])]; self.direction_publisher.publish(direction)

        if 'actuator_direction' in telemetry:
            actuator = Int8(); actuator.data = int(telemetry['actuator_direction']); self.actuator_state_publisher.publish(actuator)
    ...
    def _cmd_vel_callback(self, message: Twist) -> None:
        linear = float(message.linear.x); angular = float(message.angular.z)
        if not math.isfinite(linear) or not math.isfinite(angular):
            self.get_logger().error('Rejected non-finite /cmd_vel'); return
        half_track = self.track_width_m / 2.0
        with self._lock:
            self._requested_left = self._speed_to_command(linear - angular * half_track)
            self._requested_right = self._speed_to_command(linear + angular * half_track)
    ...
    def _brush_command_callback(self, message: Float32) -> None:
        ...
        if not math.isfinite(value):
            self.get_logger().error('Rejected non-finite /brush/command'); return
        with self._lock:
            self._requested_brush = int(round(max(0.0, min(1.0, value)) * 1000.0))
            self._last_brush_cmd_time = time.monotonic()

    def _aux_motor_command_callback(self, message: Float32) -> None:
        value = float(message.data)
        if not math.isfinite(value):
            self.get_logger().error('Rejected non-finite /aux_motor/command'); return
        with self._lock:
            self._requested_aux_motor = int(round(max(-1.0, min(1.0, value)) * 1000.0))
            self._last_aux_motor_cmd_time = time.monotonic()

    def _speed_to_command(self, speed_mps: float) -> int:
        return int(round(max(-1.0, min(1.0, speed_mps / self.max_track_speed_mps)) * 1000.0))
    ...
    def _arm_service(self, request: SetBool.Request, response: SetBool.Response) -> SetBool.Response:
        with self._lock:
            ...
            now = time.monotonic()
            self._last_cmd_time = self._last_actuator_cmd_time = self._last_brush_cmd_time = self._last_aux_motor_cmd_time = now
    ...
    def _publish_status(self) -> None:
        with self._lock:
            payload = {
                'connected': self._connected, 'dry_run': self.dry_run, 'serial_port': self.serial_port,
                'board': 'ESP32_WROOM_40PIN', 'arm_requested': self._armed_requested,
                'requested_left': self._requested_left, 'requested_right': self._requested_right,
                'requested_actuator': self._requested_actuator, 'requested_brush': self._requested_brush,
                'requested_aux_motor': self._requested_aux_motor, 'telemetry': dict(self._last_telemetry),
            }
        message = String(); message.data = json.dumps(payload, ensure_ascii=False); self.status_publisher.publish(message)
