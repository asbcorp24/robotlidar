...
// ============================================================
// Default hardware profile keeps the original internal ESP32 DAC:
//   GPIO25 -> throttle LEFT
//   GPIO26 -> throttle RIGHT
//   GPIO21 -> BTS7960 actuator RPWM
//   GPIO22 -> BTS7960 actuator LPWM
//
// RC receiver MC8RE-V2:
//   CH1 -> GPIO27 left track
//   CH2 -> GPIO33 right track
//   CH3 -> GPIO14 actuator
//   CH5 -> GPIO13 RC / SAFE / ROS
//
// HW-399 #1:
//   GPIO16 -> Reverse LEFT
...
//   GPIO19 -> Brake RIGHT
//
// HW-399 #2 (Hall, optional):
//   OUT1 -> GPIO34 Hall LEFT
//   OUT2 -> GPIO35 Hall RIGHT
//
// GM009605 OLED 0.96" 128x64:
//   SDA -> GPIO4
//   SCL -> GPIO23
//   VCC -> 3.3V
//   GND -> GND
// Uses the second ESP32 I2C controller, therefore it does not conflict
// with the optional MCP4725 profile on GPIO21/GPIO22.
// ============================================================
...
constexpr uint32_t OLED_PERIOD_MS = 200;
constexpr uint32_t OLED_PAGE_PERIOD_MS = 3000;
...
void setBrake(const Track& track, bool active) {
  ...
  if (track.side == TrackSide::Left) {
    leftBrakeActive = active;
  } else {
    rightBrakeActive = active;
  }
}
...
bool writeThrottle(const Track& track, uint16_t millivolts) {
  ...
  if (ok) {
    if (track.side == TrackSide::Left) {
      leftThrottleMv = millivolts;
    } else {
      rightThrottleMv = millivolts;
    }
  }
  return ok;
}
...
bool initializeThrottleBackend() {
  ...
  dacWrite(
      Pins::RIGHT_THROTTLE_DAC,
      millivoltsToInternalDacCode(Config::THROTTLE_DISARMED_MV));
  leftThrottleMv = Config::THROTTLE_DISARMED_MV;
  rightThrottleMv = Config::THROTTLE_DISARMED_MV;
  return true;
#endif
}
...
int16_t moveToward(
    ...
    int16_t step) {
  if (current < target) {
    return min<int16_t>(current + step, target);
  }
  if (current > target) {
    return max<int16_t>(current - step, target);
  }
  return current;
}
...
RcSnapshot readRcSnapshot() {
  ...
  result.channel1AgeUs =
      c1 ? nowUs - c1 : UINT32_MAX;
  result.channel2AgeUs =
      c2 ? nowUs - c2 : UINT32_MAX;
  result.actuatorAgeUs =
      a ? nowUs - a : UINT32_MAX;
  result.modeAgeUs =
      m ? nowUs - m : UINT32_MAX;
...
const char* modeName(ControlMode mode) {
  if (mode == ControlMode::RcManual) {
    return "RC";
  }
  if (mode == ControlMode::RosAutonomous) {
    return "ROS";
  }
  return "SAFE";
}
...
void processFrame(char* line) {
  ...
  if (!strcmp(command, "DRV")) {
    ...
    leftTrack.target =
        clampCommand(strtol(leftText, nullptr, 10));
    rightTrack.target =
        clampCommand(strtol(rightText, nullptr, 10));
    sendAck(sequence, "OK");
    return;
  }
...
void sendTelemetry(uint32_t nowMs) {
  ...
  snprintf(
      body,
      sizeof(body),
      "TEL,%lu,%d,%d,%d,%d,%d,%d,%lld,%lld,%lu,%lu,%d,%s,%u,%u,%u,0,%d,%s,%u,%d,%d,%d",
      ...
      lastRcValid ? 1 : 0,
      Config::RC_PREMIXED_BY_TRANSMITTER
          ? "BOAT_MIX"
          : "ESP32_MIX",
...
bool initializeOled() {
  ...
  if (oledDevicePresent(Config::OLED_ADDRESS_PRIMARY)) {
    oledAddress = Config::OLED_ADDRESS_PRIMARY;
  } else if (oledDevicePresent(Config::OLED_ADDRESS_SECONDARY)) {
    oledAddress = Config::OLED_ADDRESS_SECONDARY;
  } else {
...
void oledPrintMv(uint16_t mv) {
  oled.print(mv / 1000);
  oled.print('.');
  const uint16_t frac = mv % 1000;
  if (frac < 100) oled.print('0');
...
void updateOled(uint32_t nowMs) {
  ...
  const uint8_t page =
      static_cast<uint8_t>(
          (nowMs / Config::OLED_PAGE_PERIOD_MS) % 2);
  ...
  if (page == 0) {
    ...
    oled.print(F("CMD L:"));
    oled.print(leftTrack.target);
    oled.print(F(" R:"));
    oled.println(rightTrack.target);

    oled.print(F("OUT L:"));
    oled.print(leftTrack.actual);
    oled.print(F(" R:"));
    oled.println(rightTrack.actual);

    oled.print(F("DAC L:"));
    oledPrintMv(leftThrottleMv);
    oled.print(F(" R:"));
    oledPrintMv(rightThrottleMv);
    oled.println();

    oled.print(F("REV L:"));
    oled.print(leftTrack.appliedSign < 0 ? 1 : 0);
    oled.print(F(" R:"));
    oled.println(rightTrack.appliedSign < 0 ? 1 : 0);

    oled.print(F("BRK L:"));
    oled.print(leftBrakeActive ? 1 : 0);
    oled.print(F(" R:"));
    oled.println(rightBrakeActive ? 1 : 0);

    oled.print(F("ESTOP:"));
    oled.println(estopOkay() ? F("OK") : F("OPEN"));
  } else {
    oled.print(F("CH3:"));
    oled.print(lastRcSnapshot.actuatorUs);
    oled.print(F(" ACT:"));
    oled.println(actuatorName());

    oled.print(F("CH5:"));
    oled.print(lastRcSnapshot.modeUs);
    oled.print(F(" ->"));
    oled.println(modeName(controlMode));

    oled.print(F("RC:"));
    oled.println(lastRcValid ? F("OK") : F("LOST"));

#if ROBOTLIDAR_ENABLE_HALL
    oled.print(F("HALL L:"));
    oled.println(static_cast<long long>(leftTickSnapshot));
    oled.print(F("HALL R:"));
    oled.println(static_cast<long long>(rightTickSnapshot));
#else
    oled.println(F("HALL: OFF"));
    oled.println(F("GPIO34/35 FREE"));
#endif

    oled.print(F("DAC:"));
#if ROBOTLIDAR_HW_PROFILE == ROBOTLIDAR_HW_MCP4725_HW399
    oled.println(F("MCP4725"));
#else
    oled.println(F("ESP32 25/26"));
#endif
...
void setup() {
  ...
  String bootBody =
      String("BOOT,ESP32_WROOM_TRACK_CONTROLLER,9,") +
      hardwareProfileName() +
      ",30PIN_RC_CH3_GPIO14,RC_SAFE_ROS_NO_CH6,RC_CH3_BTS7960";
