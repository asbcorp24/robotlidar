...
static int rtp_send(const uint8_t *data,size_t len,void *user)
{
    (void)user;
    return platform_udp_send_video(data,len);
}
...
static void app_on_ptz_packet(const uint8_t *data,size_t len,void *user)
{
    ...
    if(flags & STCAM_PTZ_CENTER)
        ptz_center(&g_ptz);
    else
        ptz_set_target(&g_ptz,sbe16(p.pan_cdeg_be),sbe16(p.tilt_cdeg_be),speed);

    if(flags & STCAM_PTZ_REQUEST_IDR)
        platform_video_request_idr();
}
...
{
    vTaskDelay(1);
    printf("\r\nM1s stereo camera node starting\r\n");

    if(platform_init()<0){ printf("ERR platform_init\r\n"); return; }

    ptz_init(&g_ptz);
    rtp_h264_init(&g_rtp,0x4d315331u,APP_RTP_PAYLOAD_TYPE,APP_RTP_MTU);

    if(platform_servo_init()<0){ printf("ERR servo_init\r\n"); return; }
    platform_servo_write_us(ptz_pan_to_us(&g_ptz),ptz_tilt_to_us(&g_ptz));

    if(platform_wifi_connect()<0){ printf("ERR wifi_connect\r\n"); return; }
    if(platform_udp_video_open(APP_SERVER_HOST,APP_VIDEO_DST_PORT)<0){ printf("ERR udp_video_open\r\n"); return; }
    if(platform_udp_control_open(APP_PTZ_LISTEN_PORT,app_on_ptz_packet,0)<0){ printf("ERR udp_control_open\r\n"); return; }

    if(platform_video_start(app_on_h264_nal,0)<0){
        printf("WARN video backend is not running; PTZ/network loop stays active\r\n");
    }

    g_last_loop_ms=g_last_telem_ms=platform_millis();
    ...
    for(;;){
        ...
        if(now-g_last_telem_ms>=APP_TELEMETRY_PERIOD_MS){
            g_last_telem_ms=now;
            send_telemetry(now);
        }
