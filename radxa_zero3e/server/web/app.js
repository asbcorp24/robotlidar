...
async function connectStream(d){const video=$('video');$('videoPlaceholder').style.display='flex';video.src='';video.srcObject=null;if(currentPeer){currentPeer.close();currentPeer=null;}if(!d.online||!d.video_online)return;/* WebRTC relay endpoint will be connected here next. */}
...
async function detach(id){try{await api(`/api/settings/devices/${encodeURIComponent(id)}`,{method:'DELETE'});if(selected?.id===id)selected=null;await Promise.all([loadLinked(),loadDevices()]);}catch(e){alert(e.message);}}
