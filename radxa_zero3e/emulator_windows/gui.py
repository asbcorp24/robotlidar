...
class EmulatorGUI(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Radxa ZERO 3E Emulator")
        ...
        self.device_id = tk.StringVar(value="camera-win-001")
        self.device_name = tk.StringVar(value="Windows Camera Emulator")
    ...
    def _build_ui(self) -> None:
        ...
        header.pack(fill="x")
        ttk.Label(header, text="Эмулятор Radxa ZERO 3E", font=("Segoe UI", 18, "bold")).pack(side="left")
        ...
        mini = [
            ("Ширина", self.width), ("Высота", self.height), ("FPS", self.fps),
            ("Bitrate kbps", self.bitrate), ("GOP", self.gop),
        ]
    ...
    def _load_existing_config(self) -> None:
        ...
        mapping = {
            ...
            "server_rtp_host": self.server_rtp_host,
    ...
    def _config_dict(self) -> dict:
        return {
            ...
            "server_http": self.server_http.get().strip(),
            "server_rtp_host": self.server_rtp_host.get().strip(),
            "server_rtp_port": int(self.server_rtp_port.get()),
    ...
    def _save_config(self) -> bool:
        try:
            ...
            if not cfg["device_id"] or not cfg["camera_name"]:
                raise ValueError("Укажи device_id и камеру")
    ...
    def _find_cameras(self) -> None:
        ...
        def worker() -> None:
            try:
                ...
                text = cp.stdout or ""
                cameras = CAM_RE.findall(text)
                if not cameras:
                    # Fallback for FFmpeg builds whose dshow output format differs.
                    cameras = re.findall(r'"([^"]+)"\s*\n.*?Alternative name.*?video', text, flags=re.I | re.S)
    ...
    def _set_cameras(self, cameras: list[str], raw: str) -> None:
    ...
    def _start(self) -> None:
        ...
        cmd = [sys.executable, "-u", str(EMULATOR_PATH), "--config", str(CONFIG_PATH)]
        try:
        ...
        self.rtp_var.set("ожидание регистрации...")
    ...
    def _stop(self) -> None:
        if self.proc and self.proc.poll() is None:
            self._append_log("[GUI] Остановка...\n")
            self.proc.terminate()
            try:
    ...
    def _drain_logs(self) -> None:
        try:
            while True:
                ...
                if m:
                    self.rtp_var.set(f"UDP {m.group(1)}")
