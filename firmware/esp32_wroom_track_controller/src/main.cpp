...
// RobotLidar ESP32-WROOM 30-pin controller
// LEGACY wiring (default):
...
// RC: CH1=GPIO27, CH2=GPIO33, CH3=GPIO14, CH4=GPIO5, CH5=GPIO13
// HW-399 #1: GPIO16/17 Reverse L/R, GPIO18/19 Brake L/R
// HW-399 #2: Hall OUT1=GPIO34, OUT2=GPIO35
// Brush: GPIO15 -> HW-399 -> Brake; MCP4725 0x60 on SDA=4/SCL=23
// E-STOP: GPIO32 -> NC -> GND
// OLED GM009605: SDA=GPIO4, SCL=GPIO23, 3.3V
...
constexpr uint32_t AUX_WATCHDOG_MS = 500;
...
// brush_controller.cpp API
void initializeBrushController();
void updateBrushController();
void stopBrushController();
void setBrushRosCommand(uint16_t command);
uint16_t getBrushRcPulseUs();
bool getBrushRcValid();
uint16_t getBrushRosCommand();
uint16_t getBrushThrottleMv();
bool getBrushBrakeActive();
bool getBrushMcpReady();
...
void setBrake(const Track& track, bool active) {
  ...
  if (track.side == TrackSide::Left) leftBrakeActive = active;
  else rightBrakeActive = active;
}
void setReverse(Track& track, bool reverse) {
  ...
  if (track.side == TrackSide::Left) leftPulseSign = track.appliedSign;
  else rightPulseSign = track.appliedSign;
...
uint16_t millivoltsToMcp4725Code(uint16_t mv) {
  const uint32_t limited = min<uint32_t>(mv, Config::MCP4725_FULL_SCALE_MV);
  return static_cast<uint16_t>((limited * 4095UL + Config::MCP4725_FULL_SCALE_MV / 2) /
                               Config::MCP4725_FULL_SCALE_MV);
}
bool writeMcp4725(uint8_t address, uint16_t mv) {
  const uint16_t value = millivoltsToMcp4725Code(mv);
  Wire.beginTransmission(address);
...
uint8_t millivoltsToInternalDacCode(uint16_t mv) {
  const uint32_t limited = min<uint32_t>(mv, Config::INTERNAL_DAC_FULL_SCALE_MV);
  return static_cast<uint8_t>((limited * 255UL + Config::INTERNAL_DAC_FULL_SCALE_MV / 2) /
                              Config::INTERNAL_DAC_FULL_SCALE_MV);
}

bool writeThrottle(const Track& track, uint16_t mv) {
  bool ok = false;
#if ROBOTLIDAR_HW_PROFILE == ROBOTLIDAR_HW_MCP4725_HW399
  ...
  ok = writeMcp4725(address, mv);
  if (!ok) throttleBackendReady = false;
#else
  dacWrite(track.throttlePin, millivoltsToInternalDacCode(mv));
  ok = true;
#endif
  if (ok) {
    if (track.side == TrackSide::Left) leftThrottleMv = mv;
    else rightThrottleMv = mv;
  }
  return ok;
}
...
void updateActuator(const RcSnapshot& rc, uint32_t nowMs) {
#if ROBOTLIDAR_ENABLE_RC_ACTUATOR
  if (!estopOkay() || controlMode == ControlMode::Safe) {
    stopActuatorOutput();
  ...
  if (controlMode == ControlMode::RcManual) {
    commandValid = rc.actuatorValid;
    if (commandValid) requested = requestedActuatorDirection(rc.actuatorUs);
  } else if (controlMode == ControlMode::RosAutonomous) {
    commandValid = lastAuxFrameMs != 0 && nowMs - lastAuxFrameMs <= Config::AUX_WATCHDOG_MS;
    if (commandValid) requested = rosActuatorCommand;
  }
...
bool armSystem(const char* source) {
  ...
  if (!writeThrottle(leftTrack, Config::THROTTLE_IDLE_MV) ||
...
void updateTrack(Track& track, uint32_t nowMs) {
  ...
  if (!writeThrottle(track, commandToThrottleMillivolts(track.actual))) {
    track.actual = 0;
    setBrake(track, true);
    return;
  }
...
RcSnapshot readRcSnapshot() {
  ...
  c1 = rcChannel1.lastPulseUs;
  c2 = rcChannel2.lastPulseUs;
  a = rcActuator.lastPulseUs;
  m = rcMode.lastPulseUs;
  ...
  result.valid = pulseValid(result.channel1Us, result.channel1AgeUs) &&
                 pulseValid(result.channel2Us, result.channel2AgeUs) && result.modeValid;
  return result;
}
...
void changeMode(ControlMode nextMode) {
  ...
  actuatorGuardUntilMs = 0;
  actuatorNeutralSeen = false;
  rosActuatorCommand = 0;
  setBrushRosCommand(0);
...
void sendAck(uint32_t sequence, const char* result) {
  sendFrame("ACK," + String(sequence) + "," + result + "," +
...
bool verifyAndStripChecksum(char* line) {
  ...
  const int h = hexNibble(star[1]);
  const int l = hexNibble(star[2]);
  if (h < 0 || l < 0) return false;
  const uint8_t received = static_cast<uint8_t>((h << 4) | l);
  *star = '\0';
...
void processFrame(char* line) {
  ...
  if (!strcmp(command, "DRV")) {
    char* l = strtok_r(nullptr, ",", &save);
    char* r = strtok_r(nullptr, ",", &save);
    ...
    leftTrack.target = clampCommand(strtol(l, nullptr, 10));
    rightTrack.target = clampCommand(strtol(r, nullptr, 10));
    sendAck(sequence, "OK");
    return;
  }

  if (!strcmp(command, "AUX")) {
    char* actuator = strtok_r(nullptr, ",", &save);
    char* brush = strtok_r(nullptr, ",", &save);
    if (!actuator || !brush) { sendAck(sequence, "BAD_FORMAT"); return; }
    if (controlMode != ControlMode::RosAutonomous) { sendAck(sequence, "NOT_ROS_MODE"); return; }
    long actuatorValue = strtol(actuator, nullptr, 10);
    long brushValue = strtol(brush, nullptr, 10);
    rosActuatorCommand = actuatorValue > 0 ? 1 : (actuatorValue < 0 ? -1 : 0);
    setBrushRosCommand(static_cast<uint16_t>(constrain(brushValue, 0L, 1000L)));
    lastAuxFrameMs = millis();
    sendAck(sequence, "OK");
    return;
  }
  ...
  if (!strcmp(command, "ARM")) {
    ...
    const bool requestArm = v && atoi(v);
    if (!requestArm) {
      rosActuatorCommand = 0;
      setBrushRosCommand(0);
      disarmSystem("REMOTE");
    ...
    if (controlMode != ControlMode::RosAutonomous) { sendAck(sequence, "NOT_ROS_MODE"); return; }
    leftTrack.target = rightTrack.target = 0;
...
void sendTelemetry(uint32_t nowMs) {
  ...
  rt = rightTicks;
  lp = leftWindowPulses;
  rp = rightWindowPulses;
  leftWindowPulses = rightWindowPulses = 0;
  portEXIT_CRITICAL(&hallMux);
  uint32_t elapsed = nowMs - lastTelemetryPulseMs;
  if (!elapsed) elapsed = 1;
  lastTelemetryPulseMs = nowMs;
  const uint32_t lpps = lp * 1000UL / elapsed;
  const uint32_t rpps = rp * 1000UL / elapsed;

  char body[460];
  snprintf(body, sizeof(body),
      "TEL,%lu,%d,%d,%d,%d,%d,%d,%lld,%lld,%lu,%lu,%d,%s,%u,%u,%u,0,%d,BOAT_MIX,%u,%d,%d,%d,%u,%d,%u,%u,%d,%d",
      ...
      static_cast<long long>(rt),
      static_cast<unsigned long>(lpps),
      static_cast<unsigned long>(rpps),
      watchdogTripped ? 1 : 0,
      ...
      getBrushRosCommand(),
      getBrushThrottleMv(),
      getBrushBrakeActive() ? 1 : 0,
      getBrushMcpReady() ? 1 : 0);
...
bool initializeOled() {
  ...
  oled.setTextSize(1);
  oled.setCursor(0, 0);
  oled.println(F("RobotLidar ESP32"));
  oled.println(F("GM009605 OLED OK"));
  oled.print(F("I2C 0x")); oled.println(oledAddress, HEX);
  oled.println(F("SDA=4 SCL=23"));
  oled.display();
  return true;
}
...
void updateOled() {
  ...
  oled.print(estopOkay() ? F("EST:OK") : F("EST:STOP"));

  oled.setCursor(0, 8); oled.print(F("CH1:")); printRcValue(lastRcSnapshot.channel1Us, lastRcSnapshot.channel1AgeUs);
  oled.setCursor(66, 8); oled.print(F("CH2:")); printRcValue(lastRcSnapshot.channel2Us, lastRcSnapshot.channel2AgeUs);

  oled.setCursor(0, 16); oled.print(F("CMD:L")); oled.print(leftTrack.target);
  oled.setCursor(66, 16); oled.print(F("R:")); oled.print(rightTrack.target);

  oled.setCursor(0, 24); oled.print(F("DAC:")); printCompactVoltage(leftThrottleMv);
  oled.setCursor(66, 24); printCompactVoltage(rightThrottleMv); oled.print('V');

  oled.setCursor(0, 32); oled.print(F("RV:")); oled.print(leftTrack.appliedSign < 0 ? 1 : 0); oled.print('/'); oled.print(rightTrack.appliedSign < 0 ? 1 : 0);
  oled.setCursor(66, 32); oled.print(F("BK:")); oled.print(leftBrakeActive ? 1 : 0); oled.print('/'); oled.print(rightBrakeActive ? 1 : 0);

  oled.setCursor(0, 40); oled.print(F("CH3:")); printRcValue(lastRcSnapshot.actuatorUs, lastRcSnapshot.actuatorAgeUs);
  oled.setCursor(66, 40); oled.print(F("CH5:")); printRcValue(lastRcSnapshot.modeUs, lastRcSnapshot.modeAgeUs);

  oled.setCursor(0, 48);
#if ROBOTLIDAR_ENABLE_HALL
  oled.print(F("HL:")); oled.print(static_cast<long long>(lt));
  oled.setCursor(66, 48); oled.print(F("HR:")); oled.print(static_cast<long long>(rt));
#else
  oled.print(F("HL:OFF"));
  oled.setCursor(66, 48); oled.print(F("HR:OFF"));
#endif

  oled.setCursor(0, 56); oled.print(F("A:")); oled.print(actuatorName());
  oled.setCursor(48, 56); oled.print(F("B:")); oled.print(getBrushRosCommand());
  oled.setCursor(96, 56); oled.print(getBrushBrakeActive() ? F("BK") : F("ON"));
  oled.display();
}
#endif
...
void setup() {
  ...
  OledWire.begin(Pins::OLED_SDA, Pins::OLED_SCL);
  OledWire.setClock(Config::OLED_I2C_HZ);
#if ROBOTLIDAR_ENABLE_OLED
  initializeOled();
#endif
  initializeBrushController();
  ...
  String bootBody = String("BOOT,ESP32_WROOM_TRACK_CONTROLLER,15,") + hardwareProfileName() +
      ",30PIN,RC_SAFE_ROS,ROS_AUX_ACTUATOR_BRUSH,RC_PULLDOWN_FILTER";
#if ROBOTLIDAR_ENABLE_HALL
  bootBody += ",HALL_HW399";
#else
  bootBody += ",HALL_OFF";
...
#endif
  bootBody += getBrushMcpReady() ? ",BRUSH_MCP_OK" : ",BRUSH_MCP_OFFLINE";
  sendFrame(bootBody);
...
void loop() {
  ...
  if (nowMs - lastControlMs >= Config::CONTROL_PERIOD_MS) {
    lastControlMs = nowMs;
    updateCommandSource();
    updateActuator(lastRcSnapshot, nowMs);
    updateBrushController();
    ...
    if (controlMode == ControlMode::RosAutonomous &&
        (lastAuxFrameMs == 0 || nowMs - lastAuxFrameMs > Config::AUX_WATCHDOG_MS)) {
      rosActuatorCommand = 0;
      setBrushRosCommand(0);
    }
