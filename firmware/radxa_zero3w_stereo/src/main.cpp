...
void usage(const char* exe) {
    std::cout
        ...
        << "  --camera /dev/video0\n"
        << "  --ffmpeg /path/to/ffmpeg-rockchip/ffmpeg\n"
...
int wifiRssiDbm() {
    ...
    while (std::getline(f, line)) {
        ...
        double status = 0, quality = 0, level = -127;
        if (ss >> status >> quality >> level) return static_cast<int>(level);
    }
    return -127;
}
...
int main(int argc, char** argv) {
    ...
    if (!pan_pwm.open(cfg.pan_chip, cfg.pan_channel)) {
        std::cerr << "Failed to initialize PAN PWM: " << cfg.pan_chip << " channel " << cfg.pan_channel << "\n";
        return 3;
    }
    if (!tilt_pwm.open(cfg.tilt_chip, cfg.tilt_channel)) {
        std::cerr << "Failed to initialize TILT PWM: " << cfg.tilt_chip << " channel " << cfg.tilt_channel << "\n";
        return 4;
    }
    ...
    if (!startVideo(video, cfg)) {
        std::cerr << "Failed to start video pipeline\n";
        close(sock);
        return 8;
    }

    std::cout << "Radxa stereo node started\n"
              << "camera=" << cfg.camera << " " << cfg.width << "x" << cfg.height << "@" << cfg.fps << " MJPEG\n"
              << "video=rtp://" << cfg.server << ':' << cfg.video_port << " H.264 RKMPP\n"
              << "control=udp://0.0.0.0:" << cfg.control_port << "\n";
    ...
    while (g_run) {
        ...
        if (now - last_video_check >= std::chrono::seconds(1)) {
            ...
            if (!video.running()) {
                std::cerr << "video: ffmpeg exited, restarting\n";
                startVideo(video, cfg);
            }
        }
        ...
        if (now - last_telem >= std::chrono::milliseconds(250)) {
            ...
            t.wifi_rssi_dbm = static_cast<std::int8_t>(wifiRssiDbm());
