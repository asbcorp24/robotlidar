...
function selectDevice(d,reconnect=true){if(cardboardActive)exitCardboard();selected={...d};renderDevices();$('cameraTitle').textContent=d.name;$('cameraSubtitle').textContent=d.id;setOnline(d.online);$('panRange').value=d.pan||0;$('tiltRange').value=d.tilt||0;updatePtzLabels();updateTelemetry(d);if(reconnect)connectStream(d);}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&cardboardActive){exitCardboard();return;}if(['INPUT','TEXTAREA'].includes(document.activeElement.tagName))return;const map={arrowup:'up',w:'up',arrowdown:'down',s:'down',arrowleft:'left',a:'left',arrowright:'right',d:'right'};const k=e.key.toLowerCase();if(map[k]){e.preventDefault();move(map[k]);}});
if(token)enterApp();
